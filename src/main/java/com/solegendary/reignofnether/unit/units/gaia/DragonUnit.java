...
import com.solegendary.reignofnether.unit.controls.DragonUnitMoveControl;
...
public class DragonUnit extends EnderDragon implements Unit, AttackerUnit, RangedAttackerUnit {
    // region
    ...
    public Faction getFaction() {return Faction.GAIA;}
    ...
    public static final EntityDataAccessor<String> ownerDataAccessor =
            SynchedEntityData.defineId(DragonUnit.class, EntityDataSerializers.STRING);*/

    public String getOwnerName() { return this.entityData.get(ownerDataAccessor); }
    public void setOwnerName(String name) { this.entityData.set(ownerDataAccessor, "GAIA"); }
    public static final EntityDataAccessor<String> ownerDataAccessor =
            SynchedEntityData.defineId(DragonUnit.class, EntityDataSerializers.STRING);
    ...
    @Nullable
    public int getPopCost() {return ResourceCosts.DRAGON.population;}
    ...
    public DragonUnit(EntityType<? extends EnderDragon> entityType, Level level) {
        super(entityType, level);
        this.moveControl = new DragonUnitMoveControl(this);
    ...
    public static AttributeSupplier.Builder createAttributes() {
        return Monster.createMonsterAttributes()
                .add(Attributes.ATTACK_DAMAGE, DragonUnit.attackDamage)
                .add(Attributes.MOVEMENT_SPEED, DragonUnit.movementSpeed)
                .add(Attributes.MAX_HEALTH, DragonUnit.maxHealth)
                .add(Attributes.ARMOR, DragonUnit.armorValue);
    }
    ...
    public void performUnitRangedAttack(double x, double y, double z, float velocity) {
        ...
        LargeFireball fireball = new DragonUnitFireball(this.level, this, tx, ty, tz, EXPLOSION_POWER);
    ...
    // range bonus that an attacker gets when targeting this dragon, so that we can't just float high up out of range
    public int getAttackerRangeBonus(Mob attacker) {
        Vec2 attackerPos = new Vec2((float) attacker.getX(), (float) attacker.getZ());
        Vec2 dragonPos = new Vec2((float) this.getX(), (float) this.getZ());
        double horizDist = Math.sqrt(attackerPos.distanceToSqr(dragonPos));
        double vertiDist = Math.max(0, this.getY() - attacker.getY());

        // if we're directly under the dragon, just allow anything to attack it
        if (horizDist < 4)
