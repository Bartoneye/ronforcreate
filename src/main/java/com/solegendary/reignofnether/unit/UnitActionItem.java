    ...
    public void action(Level level) {
        ...
        for (Unit unit : actionableUnits) {
            ...
            switch (action) {
                ...
                case GARRISON -> {
                    Building selectedBuilding = BuildingUtils.findBuilding(((Entity) unit).level.isClientSide(), preselectedBlockPos);

                    if (unit.canGarrison() && (!(selectedBuilding instanceof BlazeWatchtower)
                            || (unit instanceof BlazeUnit && selectedBuilding instanceof BlazeWatchtower))) {
                        unit.getGarrisonGoal().setBuildingTarget(preselectedBlockPos);
                    } else if (!(unit instanceof BlazeUnit) && selectedBuilding instanceof BlazeWatchtower) {
                        HudClientEvents.showTemporaryMessage(I18n.get("abilities.reignofnether.blaze_watchtower.garrison_error"));
                    }
                }
                case UNGARRISON -> {
