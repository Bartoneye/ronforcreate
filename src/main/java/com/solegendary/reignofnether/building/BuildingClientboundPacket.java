...
import static com.solegendary.reignofnether.building.BuildingAction.GATE_OPEN;
import static com.solegendary.reignofnether.building.BuildingAction.GATE_SHUT;
...
public class BuildingClientboundPacket {
    ...
    public static void gateToggle(BlockPos buildingPos, String gateType) {
        PacketHandler.INSTANCE.send(PacketDistributor.ALL.noArg(),
        new BuildingClientboundPacket(gateType.equals(String.valueOf(GATE_SHUT)) ? BuildingAction.GATE_SHUT : BuildingAction.GATE_OPEN,
                gateType,
                buildingPos,
                Rotation.NONE,
                "",
                ...
                switch (action) {
                    ...
                    case GATE_OPEN -> {
                        if (building instanceof StoneGate stoneGate) {
                            stoneGate.changeStructure(Building.GateType.GATE_OPEN);
                        } else if (building instanceof HauntedGate hauntedGate) {
                            hauntedGate.changeStructure(Building.GateType.GATE_OPEN);
                        }
                    }
                    case GATE_SHUT -> {
                        if (building instanceof StoneGate stoneGate) {
                            stoneGate.changeStructure(Building.GateType.GATE_SHUT);
                        } else if (building instanceof HauntedGate hauntedGate) {
                            hauntedGate.changeStructure(Building.GateType.GATE_SHUT);
                        }
                    }
